...
`/pricing` — packages for the five services Caleb prices: **video editing**, **web development**, **paid ads**, **consultation** and **digital solutions**. Package data lives in `src/data/pricing.ts` (typed, like `services.ts`): a tier is a name, a `usd` amount, a `from` flag, and an optional one-line `detail`. `whoItsFor`, `includes` and `turnaround` were dropped from the schema on 2026-09-16 rather than filled with invented values — Caleb supplied one line per tier at most, so the schema now matches the data instead of implying fields nobody has filled.

Each service gets a small set of tiers as cards (name, price, its `detail` line where there is one, and a CTA into `/contact` with the service preselected via query string). Mark prices "from" where scope varies. A tier quoted rather than listed carries `usd: 'custom'` and renders as "Custom". A service may carry a standing `note` under its tiers — paid advertising uses one to say the ad budget is separate from the management fee.

**A "Not sure what you need?" card** closes the grid, routing to the booking embed on `/contact` (`/contact#book`). Brand positioning, marketing strategy and content strategy don't get a price card — they route here. (Consultation *is* priced now, so it is no longer an example of this.)

**Currency: one USD reference figure, not a per-market table.** Superseded 2026-09-16 — the earlier NGN + GBP pairing and its ₦/£ `localStorage` toggle are removed. The work is remote and quoted per project, so two market prices implied a precision the quote doesn't have, and the toggle was JS plus persisted state for a decision nobody needed to make. One `usd` figure per tier, formatted with `Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' })` at build time, rendered as "From $X" where scope varies and "$X" where it doesn't. Real figures landed on 2026-09-16, so the interim "figures are placeholders" note is gone; the page closes instead with Caleb's own line — "Final project pricing is confirmed after understanding the scope of work." The budget brackets on `/contact` use the same USD scale.
