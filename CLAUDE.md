...
**The accent colour rule.** `--color-accent` (`#FF4A1C`) is 3.22:1 on paper, ~2.98:1 on paper-alt — fails even the large-text floor on paper-alt. Never use bare accent for text on paper-alt or accent-wash; use accent-ink there instead, at any size. (On accent-wash bare accent measures 3.05:1 — it scrapes the 3:1 large-text floor with no margin, which is why it is banned there too, not because it is formally below the line.) On paper, bright accent passes WCAG AA for large text only (≥24px, or ≥18.66px bold) and for UI boundaries — display headings, rules, icon fills, hover states. For anything smaller — inline links, labels, small button text, white-on-accent fills — and for text on any band that isn't paper, use `--color-accent-ink` (`#CC370F`: 4.88:1 on paper, 4.51:1 on paper-alt, 4.62:1 on accent-wash, 5.10:1 white-on-fill). Do not put small text in the bright accent. Inline links in body copy are `--color-ink-body` text with a 2px `--color-accent` underline, not accent-coloured text.
